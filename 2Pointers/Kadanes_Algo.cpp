...
//M2 -> max tested on Leetcode and min tested on GFG
struct item{
    int l,r,lcont,rcont;
};
pair<int,int> min_max_SubArray(vector<int> &arr){
    int n = arr.size();
    int start = (int)(1e18);   //If max subarray can be -ve
    // int start = 0;        //If empty subarray is possible
    item val = {start,-start,0,0};
    item ans;
    for(int i=0;i<n;i++){
        int x = arr[i];
        ans.lcont = val.lcont+x;
        ans.rcont = val.rcont+x;
        ans.l = min(val.l,ans.lcont);
        ans.r = max(val.r,ans.rcont);
        if(ans.lcont>0){
            ans.lcont = 0;
        }
        if(ans.rcont<0){
...
void solve(){
    ...
    auto [mini,maxi] = min_max_SubArray(arr);
    cout<<mini<<" "<<maxi<<endl;
}
