...
struct Flow{
    ...
    bool solved;
    int flow;
    Flow(vector<pair<int, int>>* edges1, int n1, int s, int d){
        n = n1, src = s, dest = d;
        solved = false;
        flow = 0, iteration = 1;
        visited.resize(n);
    ...
    int dfs(int root, int currValue){
        ...
        for(auto i : edges[root]){
            ...
            if(visited[e1.dest] != iteration && e1.val > 0){
                int val = dfs(e1.dest, min(e1.val, currValue));
                if(val > 0){
                    e1.val -= val;
                    e2.val += val;
                    edgesT[i] = e1;
    ...
    void FordFulkersonFlow(){
        while(true){ // random shuffle before every iteration to tackle speciainty constructed cases
            int f = dfs(src, inf);
            if(f == 0)
                return;
            flow += f;
