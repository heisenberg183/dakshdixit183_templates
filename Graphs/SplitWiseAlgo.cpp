...
void solve(){
    ...
    while(!st.empty()){
        ...
        int credit = hi->first;
        string credit_person = hi->second;
        st.erase(low);
        st.erase(hi);
        ...
        string s = debit_person+ " will pay " + to_string(settledAmount) + " to " +credit_person;
        transactions.push_back(s);
        if(debit!=0){
            st.insert({debit,debit_person});
        }
        if(credit!=0){
            st.insert({credit,credit_person});
        }
        no_of_transactions++;
    }
