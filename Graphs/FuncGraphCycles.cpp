...
//Functional Graphs -> Graphs where each node has 1 outgoing edge and each connected component has 1 cycle
//This finds len of each cycle in each component
// Problem ->   https://codeforces.com/contest/1867/problem/D
// Solution -> From Um_nik (in his YT Video of this contest)
...
void solve(){
    ...
    vector<int> deg(n+1,0);   //Indegree
    for(int i=1;i<=n;i++){
        deg[g[i]]++;
    }
    //Removing nodes part of Cycle
    vi q(n+1);
    int zeroDeg=0;
    for(int i=1;i<=n;i++){
        if(deg[i]==0){
