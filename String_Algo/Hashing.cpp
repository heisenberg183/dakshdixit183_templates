...
struct StringHash {
    ...
    vector<int> M = {1000000007, 998244353};
    ...
    StringHash(string s) {
        int n = s.length();
        ps.resize(n + 1, vector<int>(K));
        pw.resize(n + 1, vector<int>(K, 1));
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j < K; j++) {
    ...
    vector<int> substrHash(int start, int end) {
        vector<int> res(K);
        for (int j = 0; j < K; j++) {
            int rem = (pw[end - start + 1][j] * ps[start][j]) % M[j];
...
void solve(){
    ...
    auto val = temp.substrHash(0,m-1);
    int count =0;
    for(int i=0;i+m-1<n;i++){
        if(val == hash.substrHash(i,i+m-1)){
            count++;
        }
    }
    cout<<count<<endl;
