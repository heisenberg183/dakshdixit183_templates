...
void extendgcd(int a, int b, vi &v){       //You pass a,b it will return integer x,y,gcd(a,b) as array of size 3 where ax+by=gcd(a,b)
    if (b == 0) {                       
        v[0] = 1;                          //v[0]=x
        v[1] = 0;                          //v[1]=y
        v[2] = a;                          //v[2]=gcd(a,b)
...
int mminv(int a, int m){        //Modulo Inverse Exist only if gcd(a,m)=1;
    vi arr(3);
    extendgcd(a, m, arr); 
    if(arr[2]!=1){
        return -1;
...
int mminvprime(int a, int m){       //If a,m are Coprime
    int ans = expo(a, m - 2, m); 
    ans = (ans%m+m)%m; 
    return ans;
}
vector<int> sieve(int n){                //O(nloglogn)   For Prime upto n(including n)
    int*arr = new int[n + 1]();
    vector<int> vect; 
    for (int i = 2; i <= n; i++){
        if (arr[i] == 0){
...
void isComposite(vector<int> &isComposite,int n){
    for(int i=0;i<=n;i++){
        if(isComposite[i]==0){
            for(int j=2*i;j<=n;j+=i){
                isComposite[j]=1;
...
int mod_div(int a, int b, int m){       //only for prime m
    a = a % m; 
    b = b % m; 
