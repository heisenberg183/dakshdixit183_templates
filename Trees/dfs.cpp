/*Daksh Dixit*/
//Problem -> Gardening Friends Codeforces
...
vector<vector<int>> depth_vertex;   //depth_vertex[x] will store a vector and will contain nodes at depth x(level order traversal)
vector<int> depth;                  //depth[v] will give depth of vertex v from root
vector<int> parent;                 


void dfs(int v, int p = -1, int cnt = 0) {          //DFS of a TREE
...
void solve(){
    ...
    //down.first.value = biggest height , down.second.value =2nd biggest height
    ...
    for(int tin=n;tin>=0;tin--){
        for(int v : depth_vertex[tin]){
            ...
            for(auto u : adj[v]){
                if(u==parent[v] || u==down[v].first.vertex){
                    continue;
                }
    ...
    for(int node = 1; node<=n; node++){
        ans = max(ans,k*max(up[node],down[node].first.value) - c*depth[node]);
    }
    cout<<ans<<endl;
